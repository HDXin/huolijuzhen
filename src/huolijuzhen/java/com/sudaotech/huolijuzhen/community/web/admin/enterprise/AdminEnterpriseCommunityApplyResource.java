...
@Path("/admin/enterprise/communityApply")
public class AdminEnterpriseCommunityApplyResource extends BusinessBaseResource {
    ...
    private Map<String, Object> packListInfo(CommunityApply comm){
    	...
    	infoMap.put("communityDiscrible", notNull(community.getDiscrible()));
    ...
    private Map<String, Object> packItemInfo(CommunityApply comm){
    	...
    	Long communityId = comm.getCommunityId();
    	Community community = CommunityService.getById(communityId);
    	infoMap.put("communityId", comm.getId());
    	infoMap.put("communityCcontent", notNull(community.getContent()));
    	infoMap.put("communityApprovalTime", sdf.format(community.getApprovalTime()));
    	infoMap.put("communityDetailUrl", HuolijuzhenConfig.getInstance().getDomainHtmlUrl() + "/static/community.html?id=" + comm.getId());
    	
    	Long createById = comm.getCreateBy();
    	AdminUser apply = adminUserService.getById(createById);
    	infoMap.put("applyBy", apply.getName());
    	infoMap.put("applyTime", sdf.format(comm.getCreateTime()));
    	
    	List<ImageInfo> listImageInfos = imageInfoService.findAllByImageTypeAndTargetId(ImageType.COMMUNITY_LIST, community.getId());
