package com.sudaotech.huolijuzhen.basic.web.app.park;
...
public class BannerSourcesResource extends BaseResource {
    ...
    public Result<Map<String, Long>> create(@Valid final BannerSources obj) {
    	if(!obj.getReqSourceType().equals(ReqSourceType.APP_PARK)){
    		logger.error("园区banner [create] 服务接收到非法请求");
        ...
        result.setLocation(String.format("/banner/sources/%s", id));
        result.setData(map);
        return result;
    }
    ...
    public Result<String> update(
    	...
    	if(obj.getReqSourceType()!=null && !obj.getReqSourceType().equals(ReqSourceType.APP_PARK)){
    		logger.error("园区banner [update] 服务接收到非法请求");
    ...
    public Result<BannerSources> get(@NotNull @PathParam("id") final Long id) {
        BannerSources obj = bannerSourcesService.getById(id, ReqSourceType.APP_PARK);
    ...
    public Result<Page<BannerSources>> find(
		...
		Page<BannerSources> page = bannerSourcesService.find(query, ReqSourceType.APP_PARK);
