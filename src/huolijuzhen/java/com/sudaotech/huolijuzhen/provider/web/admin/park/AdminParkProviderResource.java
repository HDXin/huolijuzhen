...
@Path("/admin/park/provider")
public class AdminParkProviderResource extends BaseResource {
    ...
    public Result<Map<String, Long>> create(@Valid final Provider obj) {
    	try{
    		...
    		temp.setCreateSide(CreateSide.PARK);
    		temp.setOperator(userId);
    		
    		temp.setParkId(getSessionQuietly().getPlatformId());
    ...
    public Result<Map<String, Object>> get(@NotNull @PathParam("id") final Long id) {
    	try {
			...
			logger.error("园区获取服务商详情 error:{}",e);
    ...
    public Result<Map<String,Object>> find(
    	...
    	try{
    		...
    		query.setCreateSide("PARK");
    		query.setParkId(getSessionQuietly().getPlatformId());
    ...
    public Result<String> getAdminUesr() {
        ...
        String adminAccount = genCodeService.nextCodeByType(Constants.Park.SERVER_USER_PREFIX);
    ...
    private Map<String, Object> packItemInfo(Provider item){
    	...
    	infoMap.put("createSide", notNull(item.getCreateSide()));
