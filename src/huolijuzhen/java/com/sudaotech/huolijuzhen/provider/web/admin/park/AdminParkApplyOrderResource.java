...
@Path("/admin/park/applyOrder")
public class AdminParkApplyOrderResource extends BusinessBaseResource {
    ...
    public Result<String> approval(
    	...
    	try{
    		...
    		applyOrder.setOperator(getSessionQuietly().getUserId());
    		...
    		if(ApplyOrderStatus.TERMINATION.equals(applyOrderStatus)){
				...
				if(ao.getApplyOrderType() == ApplyOrderType.ORDER){
	    			//退款操作
		  	        ...
		  	        req.setAmount("10");  
		  	        req.setOrder_no("2017010301000005");
    ...
    public Result<Map<String, Object>> find(
    	...
    	try{
    		...
    		//1.WAITEXECUTOR（未执行） 2.ALREADYEXECUTOR（已执行） 3.CANCEL(已取消)
    		...
    		if(StringUtils.isNotBlank(applyOrderStatusStr)){
					if(StringUtils.isBlank(applyOrderTypeStr)){
	        			...
	        			}else if("CANCEL".equals(applyOrderStatusStr)){
	        				applyOrderStatusList.add(ApplyOrderStatus.CANCEL);
	        			}
    		...
    		List<Long> serviceProIds = findServiceProIdByCurrentUser();
    ...
    private List<Long> findServiceProIdByCurrentUser(){
    	Session session = getSessionQuietly();
		FindServiceProByCurrentUser condition = new FindServiceProByCurrentUser();
		...
		if(session.getUserType() == UserType.SERVICE_PROVIDER_USER){
			...
			if(platformId == 0){
				//平台服务商
				condition.setCurrentGrade(ServiceGrade.PLATFORMSERVICE);
				condition.setServiceGradeId(provider.getId());
			}else{
				Long parkId = getSessionQuietly().getPlatformId();
				//园区服务商
				condition.setCurrentGrade(ServiceGrade.PARKSERVICE);
				condition.setServiceGradeId(provider.getId());
		...
		//2.园区管理账户
			...
			//园区自营服务
			condition.setCurrentGrade(ServiceGrade.PARKSELF);
			Long parkId = getSessionQuietly().getPlatformId();
			condition.setParkId(parkId);

			List<ServiceGrade> serviceGrades = new ArrayList<ServiceGrade>();
			serviceGrades.add(ServiceGrade.PARKSELF);
			condition.setServiceGradeList(serviceGrades);
		} 

		List<Long> serviceProIds = serviceProjectService.findServiceProIdByCurrentUser(condition);
