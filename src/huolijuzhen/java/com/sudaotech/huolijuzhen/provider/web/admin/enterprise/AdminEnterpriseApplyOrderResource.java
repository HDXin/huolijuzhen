...
@Path("/admin/enterprise/applyOrder")
public class AdminEnterpriseApplyOrderResource extends BusinessBaseResource {
    	...
    	try{
    		...
    		if(CollectionUtils.isNotEmpty(applyLabelItemList)){
    			...
    			for(ApplyLabel applyLabel:applyLabelItemList){
    				...
    				temp.setIsRequire(applyLabel.getIsRequire());
    ...
    @Path("/detail/{id}")
    @Produces(MediaTypeExt.APPLICATION_JSON_UTF8)
    public Result<Map<String, Object>> detail(@NotNull @PathParam("id") final Long id) {
    	try{

    		Map<String, Object> dataMap = new HashMap<String, Object>();
            ApplyOrder applyOrder = applyOrderService.getById(id);
            if(applyOrder != null){
                dataMap.put("id", applyOrder.getId());
                dataMap.put("orderPrice", applyOrder.getPrice());
                dataMap.put("applyOrderNo", applyOrder.getApplyOrderNo());
                dataMap.put("applyOrderStatus", applyOrder.getApplyOrderStatus());
            }
            
            return new Result<Map<String, Object>>(applyOrder == null ? ResultCode.NOT_FOUND:ResultCode.OK, dataMap);
    	}catch(Exception e){
    		logger.error("平台根据 ID 获取申请单、订单详情 error:{}",e);
    		return new Result<Map<String,Object>>(ResultCode.INTERNAL_SERVER_ERROR);
    	}
    ...
    private Map<String,Object> packListInfo(ApplyOrder applyOrder){
		...
		infoMap.put("mainImagePath", new ArrayList<String>());
    ...
    public Map<String, Object> packItemInfo(ApplyOrder applyOrder){
    	...
    	if(ApplyOrderType.APPLY.equals(applyOrderType)){
    		List<ApplyLabel> applyLables = applyLabelService.findAllByApplyOrderId(applyOrder);
    		updateValue(applyLables);
    		if(CollectionUtils.isNotEmpty(applyLables)){
    			dataMap.put("applyContent", applyLables);
    		}
    	}
    	...
    	dataMap.put("orderPrice", notNull(applyOrder.getPrice()));
    ...
    public Result<Map<String, Object>> tradeOrder(@QueryParam("orderId") Long orderId,
         ...
         try {
            ...
            if(payWay == PayWay.ALIPAY){
        	    tradeReq.setChannel(PaymentConstants.Channel.ALIPAY);
        	    tradeReq.setSubject("活力矩阵服务项目订单");
        	    tradeReq.setBody("申请单，共"+ao.getPrice()+"元");
    	        //交易金额
        	    tradeReq.setAmount(ao.getPrice().toString());  
            }	  
            if(payWay == PayWay.WECHATPAY){
            	tradeReq.setChannel(PaymentConstants.Channel.WX);
            	tradeReq.setBody("活力矩阵服务项目订单");
     	        //交易金额	
     	        tradeReq.setAmount(Arith.round(Arith.mul(ao.getPrice().toString(),"100"),0));  
            }
	  	    
	  	    tradeReq.setOrder_no(ao.getApplyOrderNo());
	        Map<String, Object> map =new HashMap<String, Object>();
	        map.put("payWay",payWay.name());
			map.put("result",paymentService.createDirectPay(tradeReq));
		    result.setData(map);
