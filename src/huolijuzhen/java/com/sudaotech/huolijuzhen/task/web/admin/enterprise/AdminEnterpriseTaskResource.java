package com.sudaotech.huolijuzhen.task.web.admin.enterprise;
...
@Path("/admin/enterprise/task")
public class AdminEnterpriseTaskResource extends BusinessBaseResource {
    ...
    public Result<Map<String, Long>> create(
    	...
    	try{
            ...
            //保存申报图
            List<ImageInfo> taskLocales = applyItem.getImageInfoList();
        	createImageInfo(ImageType.TASK_LOCALE, id, taskLocales);
...
//		}
		//执行现场图
		List<ImageInfo> operatroImage = imageInfoService.findAllByImageTypeAndTargetId(ImageType.TASK_OPERATOR, item.getId());
		infoMap.put("operatorImage", new ArrayList<Map<String, Object>>());
		if(CollectionUtils.isNotEmpty(operatroImage)){
			infoMap.put("operatorImage", imageInfoListToMap(operatroImage));
		}
