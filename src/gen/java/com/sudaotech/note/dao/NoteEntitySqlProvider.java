...
import com.sudaotech.note.dao.NoteEntityExample.Criterion;
import com.sudaotech.note.dao.NoteEntityExample;
import java.util.List;
import java.util.Map;

public class NoteEntitySqlProvider {

    public String countByExample(NoteEntityExample example) {
        BEGIN();
        SELECT("count(*)");
        FROM("note");
    ...
    public String deleteByExample(NoteEntityExample example) {
        BEGIN();
        DELETE_FROM("note");
        applyWhere(example, false);
        return SQL();
    ...
    public String insertSelective(NoteEntity record) {
        BEGIN();
        INSERT_INTO("note");
        
        if (record.getNoteId() != null) {
            VALUES("noteId", "#{noteId,jdbcType=BIGINT}");
        }
    ...
    public String selectByExample(NoteEntityExample example) {
        BEGIN();
        if (example != null && example.isDistinct()) {
            SELECT_DISTINCT("noteId");
        } else {
            SELECT("noteId");
        }
        SELECT("title");
    ...
    public String updateByExampleSelective(Map<String, Object> parameter) {
        NoteEntity record = (NoteEntity) parameter.get("record");
        NoteEntityExample example = (NoteEntityExample) parameter.get("example");
        
        BEGIN();
        UPDATE("note");
        
        if (record.getNoteId() != null) {
            SET("noteId = #{record.noteId,jdbcType=BIGINT}");
        }
    ...
    public String updateByPrimaryKeySelective(NoteEntity record) {
        BEGIN();
        UPDATE("note");
    ...
    protected void applyWhere(NoteEntityExample example, boolean includeExamplePhrase) {
        if (example == null) {
            return;
        }
